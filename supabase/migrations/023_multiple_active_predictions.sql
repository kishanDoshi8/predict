-- ============================================================
-- Migration: 023_multiple_active_predictions
-- Description:
--   Upgrades the system to support multiple concurrent active
--   predictions per room (up to a configurable limit).
--
--   Changes:
--     1. Add predictions_limit column to rooms
--     2. Update create_prediction RPC to enforce per-room limit
--     3. Add composite index for performance
--
-- IMPORTANT: predictions_limit is intentionally NOT writable
-- via any RPC, API, or frontend UI. It must only be updated
-- directly in the database by an administrator.
-- ============================================================

-- ============================================================
-- 1. Add predictions_limit to rooms
--    Default = 5. Only modifiable via direct DB/admin access.
-- ============================================================
alter table public.rooms
  add column predictions_limit integer not null default 5
    check (predictions_limit >= 1);

comment on column public.rooms.predictions_limit is
  'Max concurrent active (draft/locked) predictions allowed in this room. Admin-only — never exposed via RPC or frontend UI.';

-- ============================================================
-- 2. Composite index for fetching active predictions per room
--    ordered by deadline — replaces the scalar status index
--    for active-prediction queries.
-- ============================================================
create index if not exists idx_predictions_room_status_deadline
  on public.predictions (room_id, status, deadline asc);

-- ============================================================
-- 3. Update create_prediction RPC
--    Replace the single-active-prediction guard with a
--    per-room limit check using rooms.predictions_limit.
-- ============================================================
