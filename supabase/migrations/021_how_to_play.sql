-- ============================================================
-- Migration: 021_how_to_play
-- Description:
--   Track whether a player has seen the "How to Play" guide.
--   Adds has_seen_how_to_play to player_preferences, exposes it
--   in get_preferences, and provides a mark_how_to_play_seen RPC.
-- ============================================================
...
alter table public.player_preferences
  add column if not exists has_seen_how_to_play boolean not null default false;

-- ============================================================
-- RPC: get_preferences  (replace to include new field)
-- ============================================================
...
-- ============================================================
-- RPC: mark_how_to_play_seen
-- Marks the player as having seen the How to Play guide.
-- ============================================================

create or replace function public.mark_how_to_play_seen()
returns void
language plpgsql
security definer
set search_path = public, private
...
begin
  ...
  insert into public.player_preferences (player_id, has_seen_how_to_play)
  values (v_player_id, true)
  on conflict (player_id) do update
    set has_seen_how_to_play = true,
        updated_at = now();
end;
$$;
