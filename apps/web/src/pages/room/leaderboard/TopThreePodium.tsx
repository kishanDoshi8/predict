...
export function TopThreePodium({ top, currentPlayerId }: Props) {
    ...
    <div className='flex items-end justify-center gap-2 pt-4 pb-2'>
      {slots.map((entry) => {
        ...
        return (
          ...
          >
            <span className='text-xl'>{MEDALS[visualRank - 1]}</span>
            ...
            <p className={cn('text-xs tabular-nums', isFirst ? 'font-semibold' : 'text-muted-foreground')}>
