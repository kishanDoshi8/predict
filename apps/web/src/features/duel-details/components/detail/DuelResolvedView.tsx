...
const { winnerName, winnerPick, loserName, loserPick } = getResolvedParticipants(
duel,
challengerPickLabel,
opponentPickLabel,
);
...
icon={
isWin ? (
<Crown className='size-5 text-win' aria-hidden='true' />
) : (
<Trophy className='size-5 text-rank-1' aria-hidden='true' />
)
}
title={winnerName ? `${winnerName} won the duel` : "Duel resolved"}
...
<DuelAvatarTile name={winnerName} size='sm' ring />
...
<p className='truncate text-sm font-semibold'>{winnerName}</p>
<PickChip label={winnerPick} />
</div>
</div>
</div>
...
<div className='mt-2 flex items-center gap-2'>
<DuelAvatarTile name={loserName} size='md' ring={false} />
<div className='min-w-0'>
<p className='truncate text-sm font-semibold'>{loserName}</p>
...
Record: {rival.wins}-{rival.losses} ({rival.totalDuels} total duels),
net {signedPts(rival.netPoints)}
</p>
</div>
) : null}
