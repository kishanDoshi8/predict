-- Add snapshot accuracy counters and leaderboard movement support

alter table public.room_member_weekly_snapshots
  add column if not exists correct_predictions integer not null default 0,
  add column if not exists total_predictions integer not null default 0;

update public.room_member_weekly_snapshots
set correct_predictions = coalesce(correct_predictions, 0),
    total_predictions = coalesce(total_predictions, 0)
where correct_predictions is null or total_predictions is null;
