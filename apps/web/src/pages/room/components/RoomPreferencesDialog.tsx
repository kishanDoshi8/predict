...
const overrideVariant = (value: boolean | null): "default" | "outline" | "secondary" => {
...
export function RoomPreferencesDialog({ roomId }: Readonly<Props>) {
  const { data: preferences, isPending: isPreferencesPending } = usePreferences(roomId);
  const {
    mutate: updateGlobal,
    ...
    <Dialog>
      ...
      <DialogContent className='sm:max-w-xl'>
        <DialogHeader>
          ...
          <DialogDescription>
            Global defaults apply everywhere. This room can override each notification.
          </DialogDescription>
        </DialogHeader>
            ...
            <section className='space-y-3'>
              ...
              <div className='space-y-2'>
                {notificationKeys.map((key) => (
                  ...
                  >
                    <p className='text-sm'>{notificationLabels[key]}</p>
                    <Button
                      size='sm'
                      variant={preferences.global[key] ? "default" : "outline"}
                      onClick={() => handleGlobalToggle(key)}
                      disabled={isBusy}
                    >
                      {preferences.global[key] ? "On" : "Off"}
                    </Button>
                  </div>
                ))}
                  ...
                  >
                    <div className='flex flex-col'>
                      ...
                      <p className='text-xs text-muted-foreground'>
                        Effective: {preferences.effective[key] ? "On" : "Off"}
                      </p>
                    </div>
                    <Button
                      size='sm'
                      variant={overrideVariant(preferences.room_overrides[key])}
                      onClick={() => handleRoomCycle(key)}
                      disabled={isBusy}
                    >
                      {overrideLabel(preferences.room_overrides[key])}
                    </Button>
                  </div>
                ))}
              </div>
